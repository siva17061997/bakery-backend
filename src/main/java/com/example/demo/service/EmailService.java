...
public class EmailService {
    ...
    // 🔧 ONLY FIX: make mailSender optional at startup
    ...
    public void sendOtp(String toEmail, String otp) {
        if (mailSender == null) {
            System.err.println("⚠️ Mail sender not configured. OTP email skipped.");
            return;
        }
    ...
    public void sendOrderConfirmation(String toEmail, Order order, List<OrderItem> items) {
        ...
        if (mailSender == null) {
            System.err.println("⚠️ Mail sender not configured. User email skipped.");
            return;
        }
        ...
        try {
            ...
            for (OrderItem item : items) {
                ...
                body.append("   GST (").append(item.getGst()).append("%): ₹")
                        .append(String.format("%.2f", itemGst)).append("\n\n");
            }
    ...
    public void sendAdminOrderNotification(Order order, List<OrderItem> items) {
        ...
        if (mailSender == null) {
            System.err.println("⚠️ Mail sender not configured. Admin email skipped.");
            return;
        }
        ...
        try {
            ...
            message.setSubject("🆕 NEW ORDER #" + order.getId() + " | ₹"
                    + String.format("%.2f", order.getTotalAmount()) + " | " + order.getName());
            message.setText(body.toString());

            mailSender.send(message);
            System.out.println("✅ [ADMIN EMAIL] SUCCESS to " + ADMIN_EMAIL);
